...
public class SettingButtonBehaviour : MonoBehaviour
{
    ...
    private string path = "Assets/Resources/settings.json";
    ...
    void Start()
    {
        ...
        sr.Close();
        var dict = str.Trim(new char[] { ' ', '{', '}' }).Split(',').Select(part => part.Split(':')).ToDictionary(split => split[0].Trim('\"'), split => split[1]);
        Debug.Log(dict[property]);
        string jsonHack = "{\"placeholder\":" + dict[property] + "}";
        var EasyFix = JsonUtility.FromJson<randomkey>(jsonHack);
        Debug.Log(EasyFix.placeholder);
        GetComponentInChildren<TextMeshProUGUI>().text = EasyFix.placeholder.ToString();
    }

    private class randomkey{
        public KeyCode placeholder; //placeholder
    }

    // Update is called once per frame
    void Update()
    {
        if( editMode && Input.anyKeyDown ){
            for( int index = 0 ; index < KeyCodes.Length ; ++index )
            {
                if( Input.GetKeyDown( KeyCodes[index] ) )
                {
                    ...
                    string jsonupdate = JsonUtility.ToJson(new randomkey(){placeholder = KeyCodes[index]}).Replace("placeholder", property);
                    ...
                    writer.Close();
                    GetComponentInChildren<TextMeshProUGUI>().text = KeyCodes[index].ToString();
                    editMode = false;
                    MenuNButtons.SettingsChanged = true;
                    break;
                }
            }
        }
